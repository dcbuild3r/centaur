...
describe('slackbotv2', () => {
  ...
  it('renders root-DM fallback answers as Slack-native Markdown', async () => {
    ...
    await Promise.all(waits)
    const fallbackReply = slackApi.calls.find(call =>
      (call.method === 'chat.postMessage' || call.method === 'chat.update')
    ...
    const delivered = stringField(fallbackReply?.body.markdown_text)
