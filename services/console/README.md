...
| `CENTAUR_CONSOLE_ADMIN_SLACK_THREADS_ENABLED` | no | Set to `true` only for a trusted operator deployment to let admins browse all Slack-originated sessions, including private channels and DMs. Sessions remain read-only unless the admin owns them. Defaults to disabled. |
