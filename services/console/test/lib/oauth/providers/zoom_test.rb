    ...
    class ZoomTest < ActiveSupport::TestCase
      ...
      test "resolves the authenticated Zoom user" do
        http = expect_http_call(
          status: 200,
          body: { id: "zoom-user-id", email: "orbie@example.com", display_name: "Orbie" }.to_json
        ) do |request|
        ...
        assert_equal "orbie@example.com", identity[:email]
        assert_equal "Orbie", identity[:name]
        http.verify
      end
      ...
      test "exposes Zoom OAuth behavior" do
        ...
        assert_equal "zoom-orbie-general-oauth", strategy.credential_foreign_id(
          app_slug: "zoom-orbie-general-oauth", identity: { subject: "ignored" }
        )
