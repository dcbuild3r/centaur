...
async def _scheduling_handler(inp: Input, ctx: WorkflowContext) -> dict[str, Any]:
    ...
    if operation in {"find_availability", "book_meeting"}:
        if inp.requester_slack_team_id != WORLD_SLACK_TEAM_ID:
            raise ValueError(
                "manual meeting scheduling requires a verified World Slack requester"
            )
        ...
        # Resolve the authenticated requester, add them to the invitation, and
        # keep ownership on Orbie's managed calendar. The caller cannot choose
        # a different organizer through scheduling_args.
        requester_email = _resolve_requester_calendar_email(inp, slack_users)
        attendee_emails = [
            str(email).strip().lower()
            for email in args.get("attendee_emails", [])
            if str(email).strip()
        ]
        ...
        if operation == "book_meeting":
            # Orbie remains the meeting owner/API principal. Grant the
            # authenticated proposer host controls for the room itself so
            # they can manage participants and end the meeting for everyone.
            args["alternative_host_email"] = requester_email
