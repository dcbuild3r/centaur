...
ironProxy:
  secretSource: env
  # Optional exceptions during a staged migration between secret backends.
  secretSourceOverrides: {}
  # Optional fully-qualified 1Password refs when the tool repo is upstream.
  secretSourceRefs: {}
...
During a staged migration, `ironProxy.secretSourceOverrides` can route only
named placeholders to another backend. For example, this keeps the default
AWS-backed environment source while resolving the Codex credential from the
production 1Password vault:

```yaml
ironProxy:
  secretSource: env
  secretSourceOverrides:
    OPENAI_API_KEY: onepassword
  secretSourceRefs:
