...
ironProxy:
  ...
  secretTtl: 10m
  # Optional per-secret exceptions when most credentials remain elsewhere.
  secretSourceOverrides: {}
...
For a staged migration, set the default source to `env` and override only the
placeholders that should resolve from 1Password. The API mounts the bootstrap
Secret automatically when an override uses `onepassword`.

When a tool repository cannot carry an environment-specific item reference,
`ironProxy.secretSourceRefs` maps its placeholder directly to a fully-qualified
`op://` reference. This also mounts the bootstrap Secret automatically and
selects the 1Password service-account source for that placeholder.
