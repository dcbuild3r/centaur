...
| `SLACKBOTV2_DELETE_REACTION` | `slackbotv2.deleteReaction`. | Reaction that requests deletion of an Orbie-authored message; defaults to `wastebasket`. |
| `SLACKBOTV2_DELETE_ALLOWED_USERS` | `slackbotv2.deleteAllowedUsers`. | Comma- or whitespace-delimited Slack user IDs permitted to delete Orbie messages. Empty disables the feature unless a team allowlist is set. |
| `SLACKBOTV2_DELETE_ALLOWED_TEAM_IDS` | `slackbotv2.deleteAllowedTeamIds`. | Comma- or whitespace-delimited Slack team IDs. The bot verifies the reacting user with `users.info` and rejects Slack Connect strangers. Empty disables the feature unless a user allowlist is set. |
| `SLACKBOTV2_DELETE_ALLOWED_CHANNELS` | `slackbotv2.deleteAllowedChannels`. | Optional comma- or whitespace-delimited Slack conversation IDs that further restrict deletion. |
...
| `FIREWALL_MANAGER_SECRET_SOURCE`, `FIREWALL_MANAGER_SECRET_SOURCE_OVERRIDES`, `FIREWALL_MANAGER_SECRET_SOURCE_REFS`, `FIREWALL_MANAGER_SECRET_TTL`, `KUBERNETES_FIREWALL_MANAGER_SECRET_SOURCE` | `ironProxy.secretSource`, `ironProxy.secretSourceOverrides`, `ironProxy.secretSourceRefs`, `ironProxy.secretTtl`. | Default and per-placeholder secret sources/references plus cache TTL for rendered proxy config. |
