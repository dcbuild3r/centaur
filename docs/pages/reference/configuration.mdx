...
| `FIREWALL_MANAGER_SECRET_SOURCE`, `FIREWALL_MANAGER_SECRET_SOURCE_OVERRIDES`, `FIREWALL_MANAGER_SECRET_TTL`, `KUBERNETES_FIREWALL_MANAGER_SECRET_SOURCE` | `ironProxy.secretSource`, `ironProxy.secretSourceOverrides`, `ironProxy.secretTtl`. | Default and per-placeholder secret sources plus cache TTL for rendered proxy config. |
