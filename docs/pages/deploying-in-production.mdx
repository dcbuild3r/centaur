...
1. Add the bot scopes required by the Slackbot features you enable. The
   controlled Orbie-delete reaction requires `reactions:read`, `users:read`,
   and `chat:write`.
...
7. Subscribe to `app_mention` and to the message events you want Centaur to see:
   `message.channels`, `message.groups`, and `message.im`. If the controlled
   Orbie-delete reaction is enabled, also subscribe to `reaction_added`. To
   automatically join newly-created public channels, set
   `slackbotv2.autoJoinCreatedChannels` to `true` and subscribe to
   `channel_created`.
...
The Slackbot normalizes Slack `app_mention`, `message`, and reaction events plus
...
events and `channels:join` for the auto-join behavior. The delete reaction is
fail-closed until `SLACKBOTV2_DELETE_ALLOWED_USERS` or
`SLACKBOTV2_DELETE_ALLOWED_TEAM_IDS` is configured; it only deletes messages
whose author is the running Orbie bot. Do not rely on
